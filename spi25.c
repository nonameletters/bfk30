...
int spi_rdid(sFLASH_USBDriver *flash, unsigned char *readarr, int bytes)
{
...
static int spi_rems(sFLASH_USBDriver *flash, unsigned char *readarr)
{
...
static int spi_res(sFLASH_USBDriver *flash, unsigned char *readarr, int bytes)
{
...
static int probe_spi_rdid_generic(sFLASH_USBDriver *flash, int bytes)
{
	...
	if (!chip->name) {
		msg_cdbg("SF: Unsupported flash IDs: ");
		msg_cdbg("manuf %02x, jedec %04x\r\n", id1, id2);
		flash->chip = 0;
	}
	else {
		flash->chip = chip;
...
int spi_chip_erase_60(sFLASH_USBDriver *flash)
{
	int result;
...
int spi_chip_erase_62(sFLASH_USBDriver *flash)
{
	int result;
	struct spi_command cmds[] = {
	{
...
int spi_chip_erase_c7(sFLASH_USBDriver *flash)
{
	int result;
	struct spi_command cmds[] = {
	{
...
int spi_block_erase_52(sFLASH_USBDriver *flash, unsigned int addr,
 ...
 */
int spi_block_erase_d8(sFLASH_USBDriver *flash, unsigned int addr,
		       unsigned int blocklen)
{
	int result;
	struct spi_command cmds[] = {
 ...
 * 4k for PMC
 */
int spi_block_erase_d7(sFLASH_USBDriver *flash, unsigned int addr,
		       unsigned int blocklen)
{
	int result;
	struct spi_command cmds[] = {
...
int spi_block_erase_20(sFLASH_USBDriver *flash, unsigned int addr,
		       unsigned int blocklen)
{
	int result;
	struct spi_command cmds[] = {
...
int spi_block_erase_60(sFLASH_USBDriver *flash, unsigned int addr,
		       unsigned int blocklen)
{
...
int spi_block_erase_c7(sFLASH_USBDriver *flash, unsigned int addr,
		       unsigned int blocklen)
{
	if ((addr != 0) || (blocklen != flash->chip->total_size * 1024)) {
		msg_cerr("%s called with incorrect arguments\n",
...
int spi_byte_program(sFLASH_USBDriver *flash, unsigned int addr,
		     uint8_t databyte)
{
	int result;
	struct spi_command cmds[] = {
...
int spi_nbyte_read(sFLASH_USBDriver *flash, unsigned int address, uint8_t *bytes,
		   unsigned int len)
{
 ...
 */
int spi_read_chunked(sFLASH_USBDriver *flash, uint8_t *buf, unsigned int start,
 ...
 */
int spi_write_chunked(sFLASH_USBDriver *flash, const uint8_t *buf, unsigned int start,
		      unsigned int len, unsigned int chunksize)
{
	int rc = 0;
	unsigned int i, j, starthere, lenhere, towrite;
	/* FIXME: page_size is the wrong variable. We need max_writechunk_size
	 * in sFLASH_USBDriver to do this properly. All chips using
...
int default_spi_write_aai(sFLASH_USBDriver *flash, const uint8_t *buf, unsigned int start, unsigned int len)
{
	uint32_t pos = start;
	int result;
