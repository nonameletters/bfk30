...
#define I2C_DRIVER_EXT_FIELDS \
  ...
  virtual_timer_t vtm;					// inter-command timer

#define I2C_DRIVER_EXT_INIT_HOOK(i2cp) \
  { chVTObjectInit(&i2cp->vtm); chBSemObjectInit(&i2cp->sem,false); }
...
#define USB_DRIVER_EXT_FIELDS                                                 \
  struct USBHIDDriver *hid; \
  void                           *dfu; /** @brief   DFU driver pointer; */ \
  uint16_t                       alt_setting; /** @brief   Current USB alternate setting; */
